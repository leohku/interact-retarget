...
data_path = 'C:/Data/InterAct/Processed'
single_out_path = 'C:/Dev/interact-retarget/fbx_single'
multi_out_path = 'C:/Dev/interact-retarget/fbx_multi'
...
for file in bvh_files:
    ...
    # if '20231208_006_345' not in file: continue
    # if '20231208_005_345' not in file: continue
    
    bvh_file = os.path.join(data_path, file)
    
    ##############################################
    
    FBApplication().FileOpen('C:/Dev/interact-retarget/subjects/interact_character_'+subject+'.fbx')
    FBApplication().FileImport(bvh_file, True)
    
    options = FBFbxOptions(True)
    options.TakeSpan = FBTakeSpanOnLoad.kFBLeaveAsIs
    
    FBApplication().FileMerge('C:/Dev/interact-retarget/Geno.fbx', False, options)
    
    ###
    # Sets the character plot options before we do the real plot
    plotOptions = FBPlotOptions()
    plotOptions.PlotAllTakes = False  # Plot only current take
    plotOptions.PlotOnFrame = True  # Key on every frame
    plotOptions.PlotRate = 60      # Set frame rate to 60fps
    plotOptions.PlotPeriod = FBTime(0,0,0,1,0,FBTimeMode.kFBTimeMode60Frames)
    plotOptions.UseConstantKeyReducer = False
    plotOptions.RotationFilterToApply = FBRotationFilter.kFBRotationFilterNone
    FBSystem().Scene.Characters[1].PlotAnimation(FBCharacterPlotWhere.kFBCharacterPlotOnSkeleton, plotOptions)
    ###
    ...
    FBSystem().Scene.Characters[1].PropertyList.Find('ForceActorSpace').Data = False
    ...
    fbx_file = os.path.join(single_out_path, file.replace('.bvh', '.fbx'))
    ...
    FBSystem().Scene.Characters[1].PropertyList.Find('ForceActorSpace').Data = True
    ...
    fbx_file = os.path.join(multi_out_path, file.replace('.bvh', '.fbx'))

    FBApplication().FileSave(fbx_file, save_options)
    
##########################
    
takes = list(sorted(set([file.split('_')[2].replace('.bvh','') for file in bvh_files])))

for take in takes:
    
    FBApplication().FileNew()
    
    all_files = [file.replace('.bvh', '.fbx') for file in bvh_files if file.split('_')[2].replace('.bvh','') == take]
    
    for merge_file in all_files:
        options = FBFbxOptions(True)
        options.NamespaceList = merge_file.split('_')[1]
        FBApplication().FileMerge('C:/Dev/interact-retarget/fbx_multi/'+merge_file, False, options)
        
    fbx_file = os.path.join(multi_out_path, take+'.fbx')
